...
import { getGoogleTrendsScore } from "@/lib/googleTrends";
...
export async function GET(request: Request) {
  ...
  try {
    wikipediaViews = await getWikipediaViews(query.replaceAll(" ", "_"));
  } catch (error) {
  ...
  let newsMentions = -1;
  try {
    newsMentions = await getNewsMentions(query);
  } catch (error) {
  ...
  /* const allFailed = true; */ //Para debugging
